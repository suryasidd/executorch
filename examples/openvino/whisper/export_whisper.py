...
"""
Export Whisper with an MLX-style three-way split and lower each program to the
OpenVINO backend:
...
def ov_compile_specs(device: str):
    ...
    if device == "GPU":
        ...
        specs.append(CompileSpec("config", json.dumps(ov_cfg).encode()))
    return specs


def lower(exported_program, device: str):
    partitioner = OpenvinoPartitioner(compile_spec=ov_compile_specs(device))
...
def main():
    ...
    # Additive attention mask [1, 1, T_q=1, T_k=max_cache_length]. The host fills
    # this per step so the decoder graph stays shape-static.
    ...
    meta = {
        ...
        "d_model": cfg.d_model,
        "encoder_seq_len": enc_hidden.shape[1],
    }
