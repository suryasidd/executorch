...
"""
Whisper module split for the OpenVINO backend, modeled on
executorch/backends/mlx/examples/whisper/export_whisper.py.
...
No backend-specific custom ops are used. Self-attention cache updates use plain
aten index_copy_ so the graph stays OpenVINO-partitionable.
"""
...
class RemovePaddingIdxEmbeddingPass(ExportPass):
    ...
    padding_idx only affects gradients during training; at inference it produces
    an extra masked update that some backends (including OpenVINO) cannot
    partition. The weight and indices are args[0] and args[1]; args[2], when
    present, is padding_idx.
    """
...
class WhisperCrossKVProjection(nn.Module):
    """Runs once per utterance: encoder_hidden_states -> 2*L [B,H,T_enc,D] tensors."""
...
class StaticKVCache(nn.Module):
    """Plain aten static KV cache; index_copy_ at `cache_position`."""
    ...
    def update(self, cache_position, k_val, v_val):
        ...
        # index_copy_ along the seq dim (dim=2) is an aten op OpenVINO partitions.
